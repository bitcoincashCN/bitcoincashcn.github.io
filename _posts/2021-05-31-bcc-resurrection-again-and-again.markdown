...
* 1、在复活之前，需要确立一个顶层的共识，以追求实现真正的去中心化特征为愿景。

* 2、社区没有固定的运行模式，各自设计多层次的智力筛选器，以及因此形成的拥有强大脑力的小组，均可以各自独立运作。

* 3、启动支持链上原生通证，发行一种基于PoW工作量证明的NFT，每挖出6个区块，即（在第6个区块）产生一枚PoW NFT（支持刻字），由全网节点用户竞价获得，竞价所花费BCC自动转入黑洞地址销毁。

* 4、链上原子交换，将BCC转入黑洞地址销毁，可以按照1：1亿的比率自动兑换为ssBCC（一种链上原生通证），ssBCC将用于支付和打赏使用。
